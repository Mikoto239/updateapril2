...
app.post('/sendtheftdetails', async (req, res) => {
  ...
  try {
    ...
    const existingTheft = await TheftDetails.findOne({ as:uniqueId, currentlatitude, currentlongitude });
    ...
    if (existingTheft) {
      ...
      const theft = new TheftDetails({as: uniqueId, currentlatitude, currentlongitude });
      await theft.save();
